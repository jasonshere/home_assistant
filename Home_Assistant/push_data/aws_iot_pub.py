...
awshost = "a19asaujo13o93-ats.iot.us-east-1.amazonaws.com"      # Endpoint
awsport = 8883                                              # Port no.   
clientId = "RaspPI"                                     # Thing_Name
thingName = "RaspPI"                                    # Thing_Name
caPath = "root-CA.crt"                                      # Root_CA_Certificate_Name
certPath = "RaspPI.cert.pem"                            # <Thing_Name>.cert.pem
keyPath = "RaspPI.private.key"                          # <Thing_Name>.private.key
...
while 1==1:
    ...
    if connflag == True:
        ...
        #mqttc.publish("WeatherStationPressure", json.dumps(pressure), qos=1)        # topic: temperature # Publishing Temperature values
        mqttc.publish("WeatherStation", json.dumps(payload), qos=1)        # topic: temperature # Publishing Temperature values
        #mqttc.publish("WeatherStationTemperature", json.dumps(temp), qos=1)        # topic: temperature # Publishing Temperature values
        print("msg sent: {}". format(payload) ) # Print sent temperature msg on console
    else:
